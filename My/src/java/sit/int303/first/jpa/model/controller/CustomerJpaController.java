...
public class CustomerJpaController implements Serializable {
    ...
    public void create(Customer customer) throws PreexistingEntityException, RollbackFailureException, Exception {
        ...
        if (customer.getOrdersDetailList() == null) {
            customer.setOrdersDetailList(new ArrayList<OrdersDetail>());
        }
        ...
        try {
            ...
            List<OrdersDetail> attachedOrdersDetailList = new ArrayList<OrdersDetail>();
            for (OrdersDetail ordersDetailListOrdersDetailToAttach : customer.getOrdersDetailList()) {
                ordersDetailListOrdersDetailToAttach = em.getReference(ordersDetailListOrdersDetailToAttach.getClass(), ordersDetailListOrdersDetailToAttach.getOrdernumber());
                attachedOrdersDetailList.add(ordersDetailListOrdersDetailToAttach);
            }
            customer.setOrdersDetailList(attachedOrdersDetailList);
            em.persist(customer);
            if (salesrepemployeenumber != null) {
            ...
            for (OrdersDetail ordersDetailListOrdersDetail : customer.getOrdersDetailList()) {
                Customer oldCustomernumberOfOrdersDetailListOrdersDetail = ordersDetailListOrdersDetail.getCustomernumber();
                ordersDetailListOrdersDetail.setCustomernumber(customer);
                ordersDetailListOrdersDetail = em.merge(ordersDetailListOrdersDetail);
                if (oldCustomernumberOfOrdersDetailListOrdersDetail != null) {
                    oldCustomernumberOfOrdersDetailListOrdersDetail.getOrdersDetailList().remove(ordersDetailListOrdersDetail);
                    oldCustomernumberOfOrdersDetailListOrdersDetail = em.merge(oldCustomernumberOfOrdersDetailListOrdersDetail);
                }
            }
            utx.commit();
        } catch (Exception ex) {
    ...
    public void edit(Customer customer) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            List<OrdersDetail> ordersDetailListOld = persistentCustomer.getOrdersDetailList();
            List<OrdersDetail> ordersDetailListNew = customer.getOrdersDetailList();
            ...
            for (OrdersDetail ordersDetailListOldOrdersDetail : ordersDetailListOld) {
                if (!ordersDetailListNew.contains(ordersDetailListOldOrdersDetail)) {
                    if (illegalOrphanMessages == null) {
                        illegalOrphanMessages = new ArrayList<String>();
                    }
                    illegalOrphanMessages.add("You must retain OrdersDetail " + ordersDetailListOldOrdersDetail + " since its customernumber field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            List<OrdersDetail> attachedOrdersDetailListNew = new ArrayList<OrdersDetail>();
            for (OrdersDetail ordersDetailListNewOrdersDetailToAttach : ordersDetailListNew) {
                ordersDetailListNewOrdersDetailToAttach = em.getReference(ordersDetailListNewOrdersDetailToAttach.getClass(), ordersDetailListNewOrdersDetailToAttach.getOrdernumber());
                attachedOrdersDetailListNew.add(ordersDetailListNewOrdersDetailToAttach);
            }
            ordersDetailListNew = attachedOrdersDetailListNew;
            customer.setOrdersDetailList(ordersDetailListNew);
            ...
            for (OrdersDetail ordersDetailListNewOrdersDetail : ordersDetailListNew) {
                if (!ordersDetailListOld.contains(ordersDetailListNewOrdersDetail)) {
                    Customer oldCustomernumberOfOrdersDetailListNewOrdersDetail = ordersDetailListNewOrdersDetail.getCustomernumber();
                    ordersDetailListNewOrdersDetail.setCustomernumber(customer);
                    ordersDetailListNewOrdersDetail = em.merge(ordersDetailListNewOrdersDetail);
                    if (oldCustomernumberOfOrdersDetailListNewOrdersDetail != null && !oldCustomernumberOfOrdersDetailListNewOrdersDetail.equals(customer)) {
                        oldCustomernumberOfOrdersDetailListNewOrdersDetail.getOrdersDetailList().remove(ordersDetailListNewOrdersDetail);
                        oldCustomernumberOfOrdersDetailListNewOrdersDetail = em.merge(oldCustomernumberOfOrdersDetailListNewOrdersDetail);
                    }
                }
            }
            utx.commit();
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            List<OrdersDetail> ordersDetailListOrphanCheck = customer.getOrdersDetailList();
            for (OrdersDetail ordersDetailListOrphanCheckOrdersDetail : ordersDetailListOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This Customer (" + customer + ") cannot be destroyed since the OrdersDetail " + ordersDetailListOrphanCheckOrdersDetail + " in its ordersDetailList field has a non-nullable customernumber field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
